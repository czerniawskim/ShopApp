 ...
 */
final class Version20190809151537 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TABLE users (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, details_id INTEGER NOT NULL, username VARCHAR(30) NOT NULL COLLATE BINARY, password VARCHAR(20) NOT NULL COLLATE BINARY, email VARCHAR(255) NOT NULL COLLATE BINARY, reset_pass VARCHAR(40) NOT NULL, CONSTRAINT FK_1483A5E9BB1A0722 FOREIGN KEY (details_id) REFERENCES user_details (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__users AS SELECT id, details_id, username, password, email, reset_pass FROM users');
        $this->addSql('DROP TABLE users');
        $this->addSql('CREATE TABLE users (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, details_id INTEGER NOT NULL, username VARCHAR(30) NOT NULL, password VARCHAR(20) NOT NULL, email VARCHAR(255) NOT NULL, reset_pass VARCHAR(25) NOT NULL COLLATE BINARY)');
        $this->addSql('INSERT INTO users (id, details_id, username, password, email, reset_pass) SELECT id, details_id, username, password, email, reset_pass FROM __temp__users');
        $this->addSql('DROP TABLE __temp__users');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_1483A5E9BB1A0722 ON users (details_id)');
    }
