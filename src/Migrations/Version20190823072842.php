 ...
 */
final class Version20190823072842 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__opinions AS SELECT id, products_id, rate, description FROM opinions');
        $this->addSql('DROP TABLE opinions');
        $this->addSql('CREATE TABLE opinions (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, products_id INTEGER NOT NULL, user_id INTEGER NOT NULL, rate DOUBLE PRECISION NOT NULL, description CLOB NOT NULL COLLATE BINARY, CONSTRAINT FK_BEAF78D06C8A81A9 FOREIGN KEY (products_id) REFERENCES products (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_BEAF78D0A76ED395 FOREIGN KEY (user_id) REFERENCES users (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO opinions (id, products_id, rate, description) SELECT id, products_id, rate, description FROM __temp__opinions');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__opinions AS SELECT id, products_id, rate, description FROM opinions');
        $this->addSql('DROP TABLE opinions');
        $this->addSql('CREATE TABLE opinions (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, products_id INTEGER NOT NULL, rate DOUBLE PRECISION NOT NULL, description CLOB NOT NULL)');
        $this->addSql('INSERT INTO opinions (id, products_id, rate, description) SELECT id, products_id, rate, description FROM __temp__opinions');
        $this->addSql('DROP TABLE __temp__opinions');
        $this->addSql('CREATE INDEX IDX_BEAF78D06C8A81A9 ON opinions (products_id)');
        $this->addSql('DROP INDEX IDX_B3BA5A5A12469DE2');
